  ...
  (details) => {
    ...
    if (url !== lastRequest.urL) {
      ...
      debouncedSendNative({ type: 'PLAY', payload: url })
        ...
        .catch((e) =>
          chrome.tabs.sendMessage(tabId, { type: 'ERROR', payload: e.message })
        );
...
function getStorageFile(id) {
  return getCookie()
    .then((cookie) =>
      fetch('https://www.fshare.vn/api/v3/files/download?linkcode=' + id, {
        method: 'GET',
        credentials: 'include',
        headers: {
...
function getCookie() {
  return new Promise((resolve) => {
    chrome.cookies.get(
      { name: 'fshare-app', url: 'https://www.fshare.vn/' },
